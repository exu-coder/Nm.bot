...
document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') loadApplications(); });
...
function filtered() {
  ...
  return applications.filter(a => {
    const hay = [a.id,a.full_name,a.name,a.phone,a.email,a.course,a.message].filter(Boolean).join(' ').toLowerCase();
...
function render() {
  ...
  for (const a of list) {
    ...
    tr.innerHTML = `<td><div class="name">${esc(a.full_name||a.name||'Unknown')}</div><div class="sub">ID ${esc(String(a.id||'—'))}</div></td><td>${esc(a.phone||'—')}<div class="sub">${esc(a.email||'')}</div></td><td>${esc(a.course||'—')}</td><td><span class="badge ${esc(status)}">${esc(status)}</span></td><td>${formatDate(a.created_at)}</td><td><div class="actions"><button class="ghost small view">View</button><button class="ghost small approve">✓</button><button class="ghost small reject">×</button></div></td>`;
...
function showDetail(a) {
  ...
  $('detailBody').innerHTML = `<div class="detail-grid"><div class="detail-item"><b>APPLICATION ID</b><span class="application-id">${esc(a.id||'—')}</span></div><div class="detail-item"><b>FULL NAME</b><span>${esc(a.full_name||a.name||'—')}</span></div><div class="detail-item"><b>PHONE</b><span>${esc(a.phone||'—')}</span></div><div class="detail-item"><b>EMAIL</b><span>${esc(a.email||'—')}</span></div><div class="detail-item"><b>COURSE</b><span>${esc(a.course||'—')}</span></div><div class="detail-item"><b>STATUS</b><span>${esc(normalizeStatus(a.status))}</span></div><div class="detail-item"><b>SUBMITTED</b><span>${formatDate(a.created_at)}</span></div><div class="detail-item message"><b>MESSAGE / ADDRESS</b><span>${esc(a.message||'No message')}</span></div></div><div class="status-actions"><button class="primary" id="dApprove">Approve</button><button class="ghost" id="dReject">Reject</button></div>`;
